# SQL social media DB
This project is a Social Media Database System designed using SQL.
It focuses on building the core relational structure required for any social media platform, including users, posts, interactions, and login tracking.

The project demonstrates the foundations of database design, schema creation, and data relationships using SQL constraints such as primary keys, foreign keys, unique values, timestamps, and default values.
